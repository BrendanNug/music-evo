(function(global, factory) {
  ...
  function SMF() {
    ...
    if (arguments.length == 3 && typeof arguments[1] == "boolean") {
      ...
      if (typeof arguments[0] == 'string' && arguments[0] != '0' && arguments[0] != '1' && arguments[0] != '2') {
        console.log("here");
        self.load(arguments[0], arguments[2]);
        return self;
      }
  ...
  SMF.prototype.load = function(s, increase) {
    var off = 0;
  ...
  SMF.prototype.loadSMF = function(s, off, increase) {
    ...
    while (p < s.length - 8) {
      ...
      console.log("Chunk Call" + increase);
      this.push(new Chunk(type, data, offset, increase));
  ...
  function Chunk(t, d, off, increase) {
    console.log("In Chunk" + increase);
    if (!(this instanceof Chunk)) {
      return new Chunk(t, d, off, increase);
    }
    var i;
    if (this.sub[t]) return this.sub[t](t, d, off, increase);
  ...
  Chunk.prototype.copy = function() { return new Chunk(this.type, this.data, off, increase); };

  Chunk.prototype.sub = {
    'MTrk': function(t, d, off, increase) { 
      console.log("MTrk Call" + increase); 
      return new MTrk(d, off, increase); 
    }
  };
  ...
  function MTrk(s, off, increase) { 
    console.log("In MTrk " + increase); 
    if (!(this instanceof MTrk)) return new MTrk(s, off, increase);
    this._off = off;
    this._orig = this;
    this._tick = 0;
    if(typeof s == 'undefined') {
      this.push(new Event(0, '\xff\x2f', '', increase));
      return;
    }
    console.log("In MTrk " + increase); 
    var t = 0;
    ...
    while (p < s.length) {
      ...
      if (s.charCodeAt(p) == 0xff) {
        ...
        p += m[0];
        this.push (new Event(t, st, s.substr(p, m[1]), offset, increase));
        p += m[1];
      }
        ...
        this.push(new Event(t, w, _validate_msg_data(this, s, p, m, t, offset + 1), offset, increase));
        p += m;
      }
      else if (w.charCodeAt(0) & 0x80) { // running status
        m = _msglen(w.charCodeAt(0));
        if (w.charCodeAt(0) > 0xf0) this._complain(offset, 'Unexpected MIDI message', w.charCodeAt(0).toString(16), t);
        this.push(new Event(t, w, _validate_msg_data(this, s, p, m, t, offset), offset, increase));
        p += m;
      }
    }
  }
  ...
  function Event(t, s, d, off, increase = 0) {
    var midi;
